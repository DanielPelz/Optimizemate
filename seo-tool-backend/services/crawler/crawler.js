...
class Crawler {
    ...
    async crawlInternalPages(url, socket, userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36") {
        ...
        try {
            ...
            while (queue.length > 0) {
                ...
                if (!metaData.noIndex) {
                    const screenshotBase64 = await this.createScreenshot(currentUrl);
                    this.urlCounter++;
                    const currentUrlEndTime = new Date();
                    const currentTimePerUrl = (currentUrlEndTime - currentUrlStartTime) / 1000;
                    this.timePerUrlArray.push(currentTimePerUrl);
                    const pageData = {
                        url: currentUrl,
                        title: SEO.pageTitle.getPageTitle($),
                        description: SEO.pageDescription.getPageDescription($),
                        keywords: SEO.pageKeywords.getPageKeywords($),
                        missingAltTags: SEO.pageAltTags.getImageAltTags($),
                        brokenLinks: await SEO.pageBrokenLinks.getBrokenLinks($, currentUrl, this.page),
                        metrics: {
                            responseTime: await SEO.pageAnalizeMetrics.getResponseTime(this.page, currentUrl, 10),
                            gzip: await SEO.pageAnalizeMetrics.getGzip(this.page, currentUrl),
                            cssFiles: await SEO.pageAnalizeMetrics.getCssFiles(this.page, currentUrl),
                            jsFiles: await SEO.pageAnalizeMetrics.getJsFiles(this.page, currentUrl),
                        },
                ...
                if (!metaData.noFollow) {
                    const newInternalLinks = await this.getInternalLinks(currentUrl);
